  ...
  if (a0 >= -2 && a0 <= -1) {
    if (user_state == 1) { // kernel state syscallexception_state
      ...
      case SENDMESSAGE:
        ...
        //	pushMessage(&current_process->msg_inbox,);
        ////push it inbox??
      ...
      case RECEIVEMESSAGE:
        ...
        LDST((void *)exception_state->status);
      ...
      default:
        ...
        current_process->p_s.cause =
            (current_process->p_s.cause & ~GETEXECCODE) |
        ...
        NOT (~) operator to create a bitmask where all bits are set to 1 except
        for the bits in the GETEXECCODE position, and then performs a bitwise
        AND with the existing cause field to clear those bits. | (RI <<
