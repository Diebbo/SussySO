...
void SYSCALLExceptionHandler(){
	...
	if(a0 >= -2 && a0 <= -1){
		if(user_state == 1){ //kernel state syscall
			...
			{
			case SENDMESSAGE:
				/*If the target process is in the pcbFree_h list, set the return register (v0 in µMPS3)
				...
				break;
			case RECEIVEMESSAGE:
				/*the sender process PCB address or ANYMESSAGE in a1, a pointer to an area where the nucleus will store the
