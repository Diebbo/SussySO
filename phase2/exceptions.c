...
<<<<<<< HEAD
			...
			default:
				...
				/*exception_state->cause & ~GETEXECCODE: This part clears the bits related to the exception code in the cause field. 
					It uses the bitwise NOT (~) operator to create a bitmask where all bits are set to 1 except for the bits in the GETEXECCODE position, 
					and then performs a bitwise AND with the existing cause field to clear those bits.
					| (RI << CAUSESHIFT): This part sets the exception code to RI (Reserved Instruction) by shifting the value of RI left by CAUSESHIFT bits 
					and performing a bitwise OR with the existing cause field.*/
...
=======
      ...
      default:
        ...
        /*exception_state->cause & ~GETEXECCODE: This part clears the bits
           related to the exception code in the cause field. It uses the bitwise
           NOT (~) operator to create a bitmask where all bits are set to 1
           except for the bits in the GETEXECCODE position, and then performs a
           bitwise AND with the existing cause field to clear those bits. | (RI
           << CAUSESHIFT): This part sets the exception code to RI (Reserved
           Instruction) by shifting the value of RI left by CAUSESHIFT bits and
           performing a bitwise OR with the existing cause field.*/

        // Call Program Trap exception handler
>>>>>>> cb04e003f53bb16a1055f813852bec73e03413ce
        TrapExceptionHandler();
        break;
      }
      // Returning from SYSCALL
