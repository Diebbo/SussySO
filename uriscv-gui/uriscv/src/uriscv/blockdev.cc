...
bool Block::ReadBlock(FILE *blkFile, SWord offset) {
  if (fseek(blkFile, offset, SEEK_SET) == EOF)
    // already at EOF
    return (true);
  else if (fread((void *)blkBuf, WORDLEN, BLOCKSIZE, blkFile) != BLOCKSIZE)
    // file too short
    return (true);
  else
    // all OK
...
bool Block::WriteBlock(FILE *blkFile, SWord offset) {
  if (fseek(blkFile, offset, SEEK_SET) == EOF)
    // already at EOF
    return (true);
  else if (fwrite((void *)blkBuf, WORDLEN, BLOCKSIZE, blkFile) != BLOCKSIZE)
    // some error occurred
    return (true);
  else
    // all OK
...
unsigned int DiskParams::getCylNum(void) { return (parms[CYLNUM]); }

unsigned int DiskParams::getHeadNum(void) { return (parms[HEADNUM]); }

unsigned int DiskParams::getSectNum(void) { return (parms[SECTNUM]); }

unsigned int DiskParams::getRotTime(void) { return (parms[ROTTIME]); }

unsigned int DiskParams::getSeekTime(void) { return (parms[SEEKTIME]); }

unsigned int DiskParams::getDataSect(void) { return (parms[DATASECT]); }
...
unsigned int FlashParams::getBlocksNum(void) { return (parms[BLOCKSNUM]); }

unsigned int FlashParams::getWTime(void) { return (parms[WTIME]); }
