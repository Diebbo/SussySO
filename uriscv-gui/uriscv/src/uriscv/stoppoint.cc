...
std::string Stoppoint::ToString() const {
  static const char *fmtStr = "<Stoppoint id=%u enabled=%d, access_mode=%u, "
                              "asid=0x%02x, range=[0x%08x,0x%08x]>";
  return boost::str(boost::format(fmtStr) % id % enabled % accessMode %
...
Stoppoint *StoppointSet::Probe(Word asid, Word addr, AccessMode mode,
