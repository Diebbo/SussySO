...
HIDDEN unsigned int diskDfl[DISKPNUM] = {DFLCYL,     DFLHEAD,     DFLSECT,
                                         DFLROTTIME, DFLSEEKTIME, DFLDATAS};

// default flash device header parameters (see h/blockdev.h)
HIDDEN unsigned int flashDfl[FLASHPNUM] = {
    DFLBLOCKS,
    DFLWTIME,
};
...
HIDDEN void showHelp(const char *prgName) {
  fprintf(stderr, "%s syntax : %s {-d | -f} [parameters..]\n\n", prgName,
          prgName);
  fprintf(stderr,
          "%s -d <diskfile>%s [cyl [head [sect [rpm [seekt [datas]]]]]]\n",
          prgName, MPSFILETYPE);
  fprintf(stderr,
  ...
  fprintf(stderr,
          "\tseekt = avg. cyl2cyl time (microsecs.)\t[1..%u]\t(default = %u)\n",
          MAXSEEKTIME, diskDfl[SEEKTIME]);
  fprintf(
      stderr,
      "\tdatas = sector data occupation %%\t[%u%%..%u%%]\t(default = %u%%)\n",
  ...
  fprintf(stderr,
          "\twt = avg. write time (microsecs.)\t[1..%u]\t(default = %u)\n",
          MAXWTIME, flashDfl[WTIME]);
  fprintf(stderr,
          "\t<flashfile> = flash dev. image file name\t\t(example = %s%s)\n",
          flashDflFName, MPSFILETYPE);
  fprintf(stderr, "\t<file> = file to be written\n");
  fprintf(stderr,
          "\tnote: use /dev/null as <file> to create an empty image file\n\n");
}
  ...
  if (argc < 4 || argc > 6 || strstr(argv[2], MPSFILETYPE) == NULL) {
    ...
    if (argc == 4)
      ...
      if (!error)
        ...
        fprintf(stderr,
                "%s : flash device image file parameters wrong/missing\n",
                argv[0]);
        ret = EXIT_FAILURE;
      }
    }
...
HIDDEN int writeDisk(const char *prg, const char *fname) {
  ...
  if ((dfile = fopen(fname, "w")) == NULL ||
      ...
      fwrite((void *)diskDfl, sizeof(unsigned int), DISKPNUM, dfile) !=
          DISKPNUM)
    ret = EXIT_FAILURE;
  else {
    // write empty blocks
...
// This function creates the flash device image file on the disk, prepending it
// with a header containing geometry and performance figures. Returns an
// EXIT_SUCCESS/FAILURE code
HIDDEN int writeFlash(const char *prg, const char *fname, const char *file) {
  FILE *ffile = NULL;
  FILE *rfile = NULL;
  int ret = EXIT_SUCCESS;
  ...
  if ((ffile = fopen(fname, "w")) == NULL ||
      ...
      fwrite((void *)flashDfl, sizeof(unsigned int), FLASHPNUM, ffile) !=
          FLASHPNUM)
    ret = EXIT_FAILURE;
  else {
    ...
    if ((rfile = fopen(file, "r")) == NULL)
      ...
      if (!feof(rfile))
        fprintf(stderr,
                "%s : error writing flash device file image %s : file %s "
                "truncated\n",
                prg, fname, file);
      fclose(rfile);
    }
