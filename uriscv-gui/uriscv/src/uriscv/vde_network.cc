...
netinterface::netinterface(const char *name, const char *addr, int intnum) {
  ...
  vdeconn = vdepluglib.vde_open(name, (char *)"uRISCV", NULL);
  queue = NULL;
...
unsigned int netinterface::polling() {
  ...
  do {
    if ((poll(&polldata, 1, 0)) < 0) {
      sprintf(strbuf, "poll: %s", strerror(errno));
      Panic(strbuf);
    } else if (polldata.revents & POLLIN) {
...
class netblock *netblock::getNext() { return next; }

void netblock::setNext(class netblock *p) { next = p; }

char *netblock::getContent() { return content; }

int netblock::getLen() { return len; }
...
int netblockq::empty() { return head == NULL; }
