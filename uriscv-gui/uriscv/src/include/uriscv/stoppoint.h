...
public:
  ...
  bool operator<(const AddressRange &other) const { return LessThan(other); }
...
public:
  ...
  Stoppoint(unsigned int id, const AddressRange &range, AccessMode mode)
      : id(id), enabled(true), range(range), accessMode(mode) {}
  ...
  bool Matches(Word asid, Word addr, AccessMode mode) const {
    return (enabled && range.Contains(asid, addr) && (accessMode & mode));
  }
...
public:
  ...
  bool Add(const AddressRange &range, AccessMode mode, unsigned int id,
           bool enabled = true);
  ...
  Stoppoint *Probe(Word asid, Word addr, AccessMode mode,
                   const Processor *cpu) const;

  template <typename OutputIterator>
  void GetStoppointsInRange(Word asid, Word start, Word end,
                            OutputIterator out);
  ...
  sigc::signal<void, size_t, const Stoppoint *, Word, const Processor *>
      SignalHit;
...
template <typename OutputIterator>
void StoppointSet::GetStoppointsInRange(Word asid, Word start, Word end,
                                        OutputIterator out) {
  ...
  for (it = addressMap.lower_bound(r1);
       it != addressMap.end() && (it->first < r2 || !(r2 < it->first)); ++it) {
    *out++ = it->second;
  }
}
