...
public:
  struct JsonError : public std::runtime_error {
    JsonError(const std::string &what) : std::runtime_error(what) {}
  };
  ...
  virtual void Serialize(std::string &result, bool indent = false,
...
public:
  struct KeyError : public JsonError {
    KeyError(const std::string &key) : JsonError(key) {}
  };
  ...
  void Serialize(std::string &result, bool indent = false,
                 unsigned int indentWidth = 4, unsigned int level = 0);
