...
BaseStoppointListModel::BaseStoppointListModel(StoppointSet *set,
                                               QObject *parent)
    : QAbstractTableModel(parent), stoppoints(set),
...
QString BaseStoppointListModel::formatAddressRange(const AddressRange &range) {
  ...
  saStart =
      GetSymbolicAddress(symbolTable, range.getASID(), start, false, &ofsStart);
  if (start != end)
    saEnd =
        GetSymbolicAddress(symbolTable, range.getASID(), end, false, &ofsEnd);
  else
    saEnd = NULL;
...
const char *const StoppointListModel::headers[StoppointListModel::N_COLUMNS] = {
    "Id", "Type", "ASID", "Location", "Victims"};

StoppointListModel::StoppointListModel(StoppointSet *spSet,
                                       const char *collectionName,
                                       char idPrefix, QObject *parent)
    : BaseStoppointListModel(spSet, parent), collectionName(collectionName),
      idPrefix(idPrefix), victims(spSet->Size(), 0) {
  ...
  connect(Appl()->getDebugSession(), SIGNAL(MachineRan()), this,
          SLOT(onMachineRan()));
}
...
QVariant StoppointListModel::headerData(int section,
...
bool StoppointListModel::setData(const QModelIndex &index,
  ...
  if (index.column() == COLUMN_ACCESS_TYPE && role == Qt::EditRole &&
...
void StoppointListModel::onHit(size_t spIndex, const Stoppoint *stoppoint,
...
void StoppointListModel::StoppointAdded() { victims.push_back(0); }
