...
ProcessorWindow::ProcessorWindow(Word cpuId, QWidget *parent)
  ...
  connect(dbgSession->getCpuStatusMap(), SIGNAL(Changed()), this,
          SLOT(updateStatusInfo()));
  connect(dbgSession, SIGNAL(MachineReset()), this, SLOT(onMachineReset()));
}

void ProcessorWindow::closeEvent(QCloseEvent *event) {
  Appl()->settings.setValue(
      QString("CpuWindow%1/geometry").arg((quint32)cpu->getId()),
      saveGeometry());
  Appl()->settings.setValue(
      QString("CpuWindow%1/state").arg((quint32)cpu->getId()), saveState());
  event->accept();
}
...
void ProcessorWindow::createDockableWidgets() {
  ...
  connect(tlbWidget, SIGNAL(topLevelChanged(bool)), this,
          SLOT(updateTLBViewTitle(bool)));
...
void ProcessorWindow::updateStatusInfo() {
  ...
  if (!cpu->isHalted()) {
    ...
    prevPCLabel->setText(
        str.asprintf("0x%.8X: %s", prevPC, StrInstr(prevInstr)));
  ...
  if (!cpu->isHalted()) {
    ...
    if (dbgSession->isStopped() || cpu->isIdle())
      newStatus.append(QString(" [%1]").arg(
          dbgSession->getCpuStatusMap()->getStatus(cpu->getId())));
