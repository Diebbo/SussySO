...
TerminalWindow::TerminalWindow(unsigned int devNo, QWidget *parent)
  ...
  if (savedGeometry.isValid()) {
    ...
    resize(fm.horizontalAdvance("x") * kDefaultCols,
           fm.lineSpacing() * kDefaultRows);
  }
...
TerminalStatusWidget::TerminalStatusWidget(TerminalDevice *t, QWidget *parent)
    : QWidget(parent), terminal(t), expanded(false),
      expandedIcon(":/icons/down-16.svg"), collapsedIcon(":/icons/up-16.svg") {
  ...
  connect(hwFailureCheckBox, SIGNAL(clicked(bool)), this,
          SLOT(onHardwareFailureButtonClicked(bool)));
  layout->addWidget(hwFailureCheckBox, 0, 0);

  expanderButton = new FlatPushButton(collapsedIcon, "Show Status");
  connect(expanderButton, SIGNAL(clicked()), this,
          SLOT(onExpanderButtonClicked()));
