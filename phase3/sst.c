...
void initSST() {
  ...
  ith_sst_child =
      (pcb_PTR)SYSCALL(SENDMESSAGE, (unsigned int)ssi_cpb, (unsigned)(&ith_sst_child_payload), 0);
  // same ASID!
...
void sstRequestHandler(pcb_PTR sender, int service, void *arg) {
  ...
  default:
    // error
    SYSCALL(SENDMESSAGE, (unsigned int)ssi_cpb, TERMINATE, 0);
    break;
  }
}
...
void killSST(pcb_PTR sender) {
  ...
  SYSCALL(SENDMESSAGE, (unsigned int)ssi_cpb, (unsigned)(&input_payload), 0);
